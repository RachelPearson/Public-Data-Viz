...
pop_dat <- read.csv("mid_year_population_estimates_series_UK_2000_to_2017.csv", header = TRUE) %>% 
  gather(key = variables, value = value, population_2001:population_2017) %>% 
  filter(country == "W") %>% 
  clean_names() %>% 
  mutate(year = as.numeric(substr(variables, nchar(variables)-3, nchar(variables))) + 1) %>% 
  filter(age == 0) %>% 
  group_by(year) %>% 
  summarise(pop_U1 = sum(value)) %>% 
  ungroup() 
...
cla_wales_clean_U1 <- readRDS("clean_wales_CLA_31MAR_2003_2018.rds") %>% 
  filter(count_type %in% c("female_U1", "male_U1") & la_code == "W92000004") %>%
  group_by(year) %>% 
  summarise(count_U1 = sum(as.numeric(count))) %>% 
  ungroup() %>% 
...
# possible combinations of unrounded counts:
cla_wales_clean_U1_sim <- readRDS("clean_wales_CLA_31MAR_2003_2018.rds") %>% 
  ...
  gather(key = "type", value = "count", count_1:count_9) %>% 
  spread(key = year, value = count) %>% 
  dplyr::select(-type) %>% 
  expand.grid() %>% 
  mutate(id = row_number()) %>% 
  gather(key = "year", value = "count_U1", `2003`:`2018`, convert = TRUE) %>% 
...
plot_dat <- cla_wales_clean_U1_sim %>% 
  group_by(year) %>% 
  summarise(rate_min = round(min(rate/10000),4), # get min and max rate, by year, for plotting error bars
...
ggplot(data = plot_dat, aes(x = year)) +
  geom_col(aes(y = rate_med, fill = as.factor(year))) +
  geom_errorbar(aes(ymin = rate_min, ymax = rate_max), width = 0.4, colour = "black") +
